...
# Create thumb images.
create_thumb_images(full_folder=os.path.join(os.path.dirname(__file__), 'static/image_database'),
                    thumb_folder=os.path.join(os.path.dirname(__file__), 'static/thumb_images'),
                    suffix='',
                    height=200,
                    del_former_thumb=True)

# Prepare data set.
data_loader = load_data(data_path=os.path.join(os.path.dirname(__file__), 'static/image_database'),
                        batch_size=2,
                        shuffle=False,
                        transform='default')
        ...
        if request.form['submit'] == 'upload':
            if len(request.files) == 0:
                return render_template('upload_finish.html', \
                        message='Please select a picture file!', \
                ...
                if not (f and allowed_file(f.filename)):
                    ...
                    img = cv2.imread(upload_path)   # Read the raw image
                    cv2.imwrite(os.path.join(basepath, 'static/upload_image', 'query.jpg'), img) # save to the current path
                    return render_template('upload_finish.html', \
                        message='Upload successfully!' , \
                        img_query='static/upload_image/query.jpg?123456')    # the success inferface after clicking upload

        elif request.form['submit'] == 'retrieval':
            start_time = time.time()
            # Query.
            query_image = load_query_image(upload_path)
            ...
            print(sorted_paths) # print out the image path sorted by similarity after searching.
            tmb_images = [os.path.join('static/thumb_images', os.path.split(sorted_path)[1]) for sorted_path in sorted_paths]
